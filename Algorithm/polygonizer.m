...
% return the target points within the shape
...
% 1. filter out the points located inside the holes and outside the shapes
% 2. merge the target points located inside different patterns and remove
% the target points inside the holes in the shapes
...
for k = 1:N
    if (nnz(A(:,k)) > 0)
        ...
        % points inside the holes in the shapes
        for l = find(A(:,k))'
            boundary = B{l};
            in = inpolygon(bigGridX(:), bigGridY(:), boundary(:,2), higher_x - boundary(:,1));
